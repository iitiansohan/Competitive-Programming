...
void solve(){
    ...
    if(l>=r) {cout<<s<<"\n"; return;}
    string tempr=s.substr(l,n-2*l);
    string temp=tempr;
    reverse(tempr.begin(),tempr.end());
    auto pi1=kmp(temp+'#'+tempr);
    auto pi2=kmp(tempr+'#'+temp);
    if(pi1.back()>=pi2.back()){
        cout<<s.substr(0,l+pi1.back())<<s.substr(r+1,l)<<"\n";
    }
    else{
        cout<<s.substr(0,l)<<s.substr(r-pi2.back()+1,pi2.back()+l)<<"\n";
    }  
}
