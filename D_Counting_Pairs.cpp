...
void solve() {
    ...
    for (int i = 0; i < n - 1; i++) {
        auto validPairsStart = lower_bound(a.begin() + i + 1, a.end(), low - a[i]) - a.begin();
        auto validPairsEnd = upper_bound(a.begin() + i + 1, a.end(), high - a[i]) - a.begin() - 1;

        if (validPairsStart <= validPairsEnd)
            count += (validPairsEnd - validPairsStart + 1);
    }
