...
void solve()
{
    ...
    else
    {
        ...
        int c=0;
        while (k > 0) {
            int remaining_slots = n - c;

            if (c == 0) {
                
                int collectable_cans = a[c] * remaining_slots;
                if (k <= collectable_cans) {
                    ans += k;
                    break;
                } else {
                    ans += collectable_cans;
                    k -= collectable_cans;
                }
            } else {
                
                int collectable_cans = (a[c] - a[c-1]) * remaining_slots;
                if (k <= collectable_cans) {
                    ans += k;
                    break;
                } else {
